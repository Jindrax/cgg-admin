<template>
  <q-page class="flex flex-top">
    <q-card style="width: 100%">
      <q-card-title>Crear Cliente</q-card-title>
      <q-card-separator/>
      <q-card-main>
    ...
    <q-card style="width: 100%">
      <q-card-title>Clientes</q-card-title>
      <q-card-separator/>
      <q-card-main>
        <q-search
          hide-underline
          ...
          <q-tr slot="body" slot-scope="props" :props="props">
            ...
            <q-td key="saldo" :props="props">
              <q-chip
                small
                square
                :color="props.row.saldo > 0? 'green' : 'red'"
                @click="modal_saldo(props.row.id)"
              >
                {{
                props.row.saldo }}
              </q-chip>
            ...
            <q-td key="sesion_activa" :props="props">
              <q-btn
                ...
                color="red"
                icon="cancel"
                :disable="props.row.sesion_activa==null"
                @click="logoutFromApp(props.row.sesion_activa)"
                class="q-mr-xs"
              />
            </q-td>
            <q-td v-if="usuario.admin" key="restaurar_pass" :props="props">
              <q-btn
                ...
                color="secondary"
                icon="edit"
                :disable="props.row.restaurar_pass"
                @click="restore_pass(props.row.id, props.row.username)"
                class="q-mr-xs"
              />
            </q-td>
            <q-td key="miembro" :props="props">
              <q-btn
                size="sm"
                round
                dense
                color="green"
                icon="account_circle"
                :disable="props.row.miembro==true"
                @click="comprarMembresia(props.row.id)"
                class="q-mr-xs"
              />
            </q-td>
            <q-td
              key="miembro_hasta"
              :props="props"
            >{{ props.row.miembro? formatFecha(props.row.miembro_hasta) : 'Nunca' }}</q-td>
            <q-td key="ultima_promo" :props="props">
              <q-btn
                size="sm"
                round
                dense
                ...
                @click="comprarPromo(props.row.id)"
                class="q-mr-xs"
              />
            </q-td>
            ...
            <q-td key="nacimiento" :props="props">{{ formatFecha(props.row.nacimiento) }}</q-td>
          </q-tr>
        </q-table>
        <q-btn
          icon="update"
...
export default {
  ...
  methods: {
    ...
    /**
     * Funcion que solicita la compra de una membresia por el cliente
     * @param {Number} cliente Id del cliente que solicita la membresia
     */
    comprarMembresia(cliente) {
      console.log("ComprarMembresia placeholder");
      this.socket.post(
        "/upgrademembership",
        {
          cliente: cliente
    ...
    /**
     * Funcion que solicita la compra de una promocion por el cliente
     * @param {Number} cliente Id del cliente que solicita la promocion
     */
    comprarPromo(cliente) {
      console.log("ComprarPromo placeholder");
      this.socket.post(
        "/sell_promo",
        {
          cliente: cliente
        ...
        response => {
          ...
          //Notificamos al usuario
          this.notificar(`Promocion de ${response.precio_compra} pesos, vigente hasta ${this.formatFecha(response.fecha_expiracion)}`);
        }
      );
    },
    /**
